...
TEST_F(PyIntegerTest, TestValue) {
  EXPECT_TRUE(integer1->Equal(CreateIntegerWithCString("10")));
  EXPECT_TRUE(integer2->Equal(CreateIntegerWithCString("20")));
}

TEST_F(PyIntegerTest, TestAdd) {
  auto result = IntegerKlass::Self()->add(integer1, integer2);
  EXPECT_TRUE(std::dynamic_pointer_cast<PyInteger>(result)->Equal(
...
TEST_F(PyIntegerTest, TestRepr) {
  ...
  EXPECT_TRUE(std::dynamic_pointer_cast<PyString>(result)->Equal(
    CreatePyInteger(10)
  ));
}
...
TEST_F(PyIntegerTest, TestSerialize) {
  ...
  EXPECT_EQ(bytes.Get(0), static_cast<Byte>(Literal::INTEGER));
