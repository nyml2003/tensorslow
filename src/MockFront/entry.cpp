...
void CleanTestDir() {
  ...
  system(("mkdir -p " + ToCppString(test_dir)).c_str());
}
...
int main() {
  ...
  //         instructionsObject->_serialize_(), constsObject, namesObject,
  //         varNamesObject,
  //         std::make_shared<PyString>(CreateStringWithCString("f")),
  nLocals
  //       );
  //     }
  //     List<PyObjPtr> consts;
  ...
  //       List<PyObjPtr> instructions;
  //       instructions.Push(CreateLoadFast(0));   //  instructions[0]
  //       instructions.Push(CreateLoadConst(1));  // instructions[1]
  //       instructions.Push(CreateCompareOp(CompareOp::EQUAL)
  //       );                                          // instructions[2]
  //       instructions.Push(CreatePopJumpIfTrue(5));  // instructions[3]
  //       instructions.Push(CreateLoadFast(0));       // instructions[4]
  //       instructions.Push(CreateLoadConst(2));      // instructions[5]
  //       instructions.Push(CreateCompareOp(CompareOp::EQUAL)
  //       );                                           // instructions[6]
  //       instructions.Push(CreatePopJumpIfFalse(3));  // instructions[7]
  ...
  //       List<PyObjPtr> names;
  //       names.Push(CreatePyString(CreateStringWithCString("fibonacci")));
  //       PyListPtr namesObject = std::make_shared<PyList>(names);
  ...
  //     List<PyObjPtr> consts;
  //     consts.Push(std::make_shared<PyInteger>(
  //       CreateIntegerWithString(CreateStringWithCString("20"))
  //     ));
  //     consts.Push(code);
  //     consts.Push(CreatePyString(CreateStringWithCString("fibonacci")));
  //     consts.Push(PyNone::Instance());
  //     PyListPtr constsObject = std::make_shared<PyList>(consts);

  //     List<PyObjPtr> names;
  //     names.Push(CreatePyString(CreateStringWithCString("x")));
  //     names.Push(CreatePyString(CreateStringWithCString("fibonacci")));
  //     PyListPtr namesObject = std::make_shared<PyList>(names);
  ...
  //     PyListPtr varNamesObject =
  std::make_shared<PyList>(List<PyObjPtr>());
...
int main() {
  // 获取任务管理器的单例实例
  auto& taskManager = torchlight::MockFront::TaskManager::Instance();

  // 运行所有已启用的任务
  std::cout << "Running all enabled tasks..." << std::endl;
  taskManager.RunAllEnabledTasks();
